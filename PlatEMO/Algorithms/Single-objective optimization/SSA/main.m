%%  闲鱼：深度学习与智能算法
%%  唯一官方店铺：https://mbd.pub/o/autOOAr-aWWbm3BtZw==
...
%%  调用算法
[SSA_score,SSA_pos,SSA_curve]=SSA(SearchAgents,Max_iterations,lb,ub,dim,fobj);    % 调用SSA算法
[OOA_score,OOA_pos,OOA_curve]=OOA(SearchAgents,Max_iterations,lb,ub,dim,fobj);    % 调用OOA算法
...
if ~strcmp(number,'F16')&&~strcmp(number,'F9')&&~strcmp(number,'F11')  %这里是因为这几个函数收敛太快，不适用于semilogy，直接plot
    ...
    hold on
    semilogy(iter(k),OOA_curve(k),'r-p','linewidth',1);
else
    plot(iter(k),SSA_curve(k),'b-*','linewidth',1);
    hold on
    plot(iter(k),OOA_curve(k),'r-p','linewidth',1);
end
grid on;
title('收敛曲线')
xlabel('迭代次数');
