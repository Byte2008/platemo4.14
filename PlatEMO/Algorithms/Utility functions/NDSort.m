...
%--------------------------------------------------------------------------
%%NDSort （Non-dominated Sort）是一个高效的非支配排序方法，用于进化多目标优化算法中对解集进行分层排序。
% 核心功能：该方法根据帕累托支配关系将解集划分为多个非支配前沿： 第一前沿 ：不被任何其他解支配的解， 第二前沿 ：被第一前沿支配，但不被其他解支配的解， 以此类推...
%输入参数：PopObj 矩阵 (N×M) 目标值矩阵，N为解的数量，M为目标数量；PopCon (可选) 矩阵 (N×C)
%约束违反矩阵，用于约束支配；nSort 标量 至少需要排序的解的数量
%输出参数：FrontNo 向量 (1×N) 前沿编号向量，FrontNo(i) 表示第i个解的前沿编号；MaxFNo 标量 最大前沿编号（不包括inf）
%% 调用实例：无约束情况，只找第一前沿[FrontNo,MaxFNo] = NDSort(PopObj,1);
% 有约束情况，排序整个种群[FrontNo,MaxFNo] = NDSort(PopObj,PopCon,inf);
    PopObj = varargin{1};
    [N,M]  = size(PopObj);
    if nargin == 2
        nSort  = varargin{2};
