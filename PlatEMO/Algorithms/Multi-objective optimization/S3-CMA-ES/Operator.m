...
% This function is written by Huangke Chen
    ...
    %边界处理：对采样向量arx作截断到[0,10]，再回填到个体并整体评估。
    arx         = repmat(xmean,1,lambda) + sigma*B*(repmat(D,1,lambda).*randn(N,lambda));
    arx(arx<0)  = 0;
    arx(arx>10) = 10;
    %在给定bestmem与固定PV的条件下，仅对当前分组的DV维度采样与更新；适应度使用目标平方和作为标量聚合以便CMA-ES排序与重组。
    ...
    %完整CMA-ES步骤：选择-重组（权重均值）-路径累计-协方差更新（rank-1与rank-μ）-步长自适应-特征分解更新。
